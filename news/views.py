...
def get_info(posts):
    ...
    for post in posts:
        ...
        print(article_id)
        ps = FullPost()
        article = ps.get_half_post(article_id)
        print(article[1][0])
        list_articles.append(article[1][0])
    print(list_articles)
...
def index(request):
    ...
    if request.method == "GET":  # 요청하는 방식이 GET 방식인지 확인하기
        ...
        if user_name:  # 로그인 한 사용자라면
            ...
            post = MyPost()
            my_post4_2 = list(get_post_safe(post.get_top_articles_today(), 3)[1]) #오늘 올라온 조회수 높은 3개 기사 조회
            post = MyPost()
            ...
            my_post4 = my_post4[1:]
            my_post5 = list(get_post_safe(post.get_latest_articles_by_category(), 8)[1]) #분야별 전체 기자들이 쓴 최신 기사 3개
            print(my_post5)
            post = MyPost()
            my_post5_1 = list(get_post_safe(post.get_unread_top_articles_today_by_category(user_id), 8)) #분야별 내가 읽지 않은 오늘 올라온 조회수 높은 기사 3개
            post = MyPost()
            my_post5_2 = [] 
            my_post5_3 = []
            my_post5_4 = []
            ...
            my_post6 = list(get_post_safe(post.getHotPostToday(), 1)[1]) #전체 기자들이 쓴 기사 중 작성일이 하루가 지나지 않은 기사 중 조회수 가장 높은 기사
            post = MyPost()
            my_post7 = list(get_post_safe(post.getHotPostWeek(), 2)[1]) #전체 기자들이 쓴 기사 중 작성일이 일주일이 지나지 않은 기사 중 조회수 높은 순으로 2개
            post = MyPost()
            my_post8 = get_post_safe(post.get_unread_popular_article(user_id), 1) #구독한 기자들의 읽지 않은 조회수 높은 기사'
            '''
            my_post1 = get_info(my_post1)
            print("my_post2")
            my_post2 = get_info(my_post2)
            print("my_post3")
            my_post3 = get_info(my_post3)
            print("my_post4")
            my_post4 = get_info(my_post4)
            print("my_post5")
            my_post5 = get_info(my_post5)
            print("my_post4_1")
            my_post4_1 = get_info(my_post4_1)
            print("my_post5_1")
            my_post5_1 = get_info(my_post5_1)
            print("my_post6")
            my_post6 = get_info(my_post6)
            print("my_post7")
            my_post7 = get_info(my_post7)
            print("my_post8")
            my_post8 = get_info(my_post8)
            '''
            for post in my_post5:
                print(post)
                if post[0] == 'politics':
                    my_post5_2.append(post) 
                if post[0] == 'economy':
                    my_post5_3.append(post)
                if post[0] == 'entertainments':
                    my_post5_4.append(post)
                if post[0] == 'sport':
                    my_post5_5.append(post)
                if post[0] == 'IT':
                    my_post5_6.append(post)
            my_post5_2_1 = random.choice(my_post5_2)
            my_post5_2.remove(my_post5_2_1)
            my_post5_3_1 = random.choice(my_post5_3)
            my_post5_3.remove(my_post5_3_1)
            my_post5_4_1 = random.choice(my_post5_4)
            my_post5_4.remove(my_post5_4_1)
            my_post5_5_1 = random.choice(my_post5_5)
            my_post5_5.remove(my_post5_5_1)
            my_post5_6_1 = random.choice(my_post5_6)
            my_post5_6.remove(my_post5_6_1)
            post_dict = {'post1':my_post1, 'post2':my_post2, 'post3':my_post3, 'post4':my_post4, 'post4_1':my_post4_1, \
                         'post5_2_1':my_post5_2_1, 'post5_3_1':my_post5_3_1, 'post5_4_1':my_post5_4_1, 'post5_5_1':my_post5_5_1, 'post5_6_1':my_post5_6_1, \
                         'post5':my_post5, 'post5_1':my_post5_1, 'post5_2':my_post5_2, 'post5_3':my_post5_3, 'post5_4':my_post5_4, 'post5_5':my_post5_5, 'post5_6':my_post5_6, \
                         'post6':my_post6, 'post7':my_post7, 'post8':my_post8}
    #post = FullPost()
    #full_post1 = post
    print(post_dict)
...
def detail_page(request, article_id):
    if request.method == "GET":
        ...
        if user_name:  # 로그인 한 사용자라면
            post = FullPost()
            article = post.get_full_post(article_id)[1]
            sub = Subscribe()
            jid = sub.get_journalist_id(article_id)[1][0]
            sub = Subscribe()
            cnt_sub = sub.get_subscriber_count(jid)[1]
            print(article[0][0])
            return render(request, 'news/detail-page.html', {'today': today, 'day': day, 'article': article[0], 'cnt_sub': cnt_sub})
        else:
            return render(request, 'news/login.html', {'today': today, 'day': day})
